...
class DeepGuidedFilterMaskRefiner(nn.Module):
    def __init__(self, hid_channels=16):
        super().__init__()
        self.box_filter = nn.Conv2d(1, 1, kernel_size=3, padding=1, bias=False, groups=1)
        self.box_filter.weight.data[...] = 1 / 9
        self.conv = nn.Sequential(
            nn.Conv2d(1 * 2 + hid_channels, hid_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(hid_channels),
            nn.ReLU(True),
            nn.Conv2d(hid_channels, hid_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(hid_channels),
            nn.ReLU(True),
            nn.Conv2d(hid_channels, 1, kernel_size=1, bias=True)
        )
        
    def forward_single_frame(self, fine_src, base_src, base_pha, base_hid):
        fine_x = fine_src.mean(1, keepdim=True)
        base_x = base_src.mean(1, keepdim=True)
        base_y = base_pha
    ...
    def forward_time_series(self, fine_src, base_src, base_pha, base_hid):
        B, T = fine_src.shape[:2]
    ...
    def forward(self, fine_src, base_src, base_pha, base_hid):
        if fine_src.ndim == 5:
            return self.forward_time_series(fine_src, base_src, base_pha, base_hid)
        else:
            return self.forward_single_frame(fine_src, base_src, base_pha, base_hid)
